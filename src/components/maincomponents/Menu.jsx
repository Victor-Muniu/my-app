...
const Menu = ({ table, onClose }) => {
  ...
  const addToOrder = (item) => {
    setCurrentOrder((prevState) => {
      const existingItem = prevState.find((orderItem) => orderItem.id === item._id);
      if (existingItem) {
  ...
  const updateQuantity = (itemId, newQuantity) => {
    if (newQuantity === 0) {
      setCurrentOrder((prevState) => prevState.filter((item) => item.id !== itemId));
  ...
  const updateOrderToPost = (itemId, itemName, quantity, remove = false) => {
    setOrderToPost((prevState) => {
      const existingItemIndex = prevState.menu_items.findIndex((item) => item.name === itemName);
      if (existingItemIndex > -1) {
        if (remove) {
          return {
            ...prevState,
            menu_items: prevState.menu_items.filter((item) => item.name !== itemName)
          };
        }
        return {
          ...prevState,
  ...
  const sendToKitchen = async () => {
    try {
      await axios.post("http://localhost:3002/assign-table", orderToPost, {
        withCredentials: true,
      });
